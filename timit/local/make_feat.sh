...
feat_type=$1
compress=true
data_dir=/home/fran/Documents/CTC_pytorch_data/data_prepare
...
if [ "$feat_type" == "Fbank" ]; then
	...
	for x in train dev test; do
        compute-fbank-feats-cuda --config=$fbank_config scp,p:$data_dir/$x/wav_sph.scp ark:- | \
...
if [ "$feat_type" == 'MFCC' ]; then
	...
	for x in train dev test; do
        compute-mfcc-feats-cuda --config=$mfcc_config scp,p:$data_dir/$x/wav_sph.scp ark:- | \
